...
const TrainersSlider = () => {
    ...
    return (
        <div className='relative flex items-center group pb-10 my-10'>
            <MdChevronLeft
                onClick={ slideLeft }
                className='bg-white left-0 rounded-full absolute opacity-50 hover:opacity-100 cursor-pointer z-10 hidden group-hover:block md:hidden'
                size={ 40 }
            />
            <div
                id={ 'slider' + rowID }
                className='w-full h-full overflow-x-scroll whitespace-nowrap relative'
                style={ { scrollbarwidth: "none", overflowX: "hidden" } } // Hide scrollbar in Firefox and hide horizontal scroll
                scrollbarwidth="none" // Hide scrollbar in Webkit-based browsers
            >
                { trainers.map((trainer, index) => (
                    <div className='w-[160px] sm:w-[200px] md:w-[240px] lg:w-[280px] inline-block cursor-pointer relative p-2' key={ index } onClick={ () => showTrainerDetails(trainer) }>
                        ...
                        />
                        <h2 className="mt-2 text-base font-semibold capitalize text-gray-300 dark:text-white">
            ...
            <MdChevronRight
                onClick={ slideRight }
                className='bg-white right-0 rounded-full absolute opacity-50 hover:opacity-100 cursor-pointer z-10 hidden group-hover:block md:hidden'
                size={ 40 }
            />
        </div>
    );
